...
```bash
dotnet new sln -n Hopen
dotnet new classlib -n Hopen.Core
dotnet new classlib -n Hopen.Application
dotnet new classlib -n Hopen.Infrastructure
dotnet new webapi -n Hopen.Api

dotnet sln add Hopen.Core Hopen.Application Hopen.Infrastructure Hopen.Api
```

**2. Configure project dependencies:**
```
Hopen.Core          → (no dependencies)
Hopen.Application   → Hopen.Core
Hopen.Infrastructure → Hopen.Core, Hopen.Application
Hopen.Api           → Hopen.Application, Hopen.Infrastructure
```

**3. Install NuGet packages:**
```bash
# Hopen.Infrastructure
...
```csharp
// Program.cs
builder.Services.AddDbContext<HopenDbContext>(options =>
    options.UseNpgsql(builder.Configuration.GetConnectionString("Default")));

// Create migration
dotnet ef migrations add InitialCreate --project Hopen.Infrastructure --startup-project Hopen.Api

// Apply migration
dotnet ef database update --project Hopen.Infrastructure --startup-project Hopen.Api
```

**5. Register repositories and services:**
```csharp
// Hopen.Api/Configuration/ServiceCollectionExtensions.cs
...
```bash
ng new hopen-admin --standalone --routing --style=scss
cd hopen-admin
```

**2. Create directory structure:**
```bash
mkdir -p src/app/{pages,features,shared,core}
mkdir -p src/app/pages/content-types/{list,edit}
mkdir -p src/app/features/content-types/{services,models,components}
mkdir -p src/app/shared/{components,services,pipes,directives}
mkdir -p src/app/core/{auth,layout,config}
```

**3. Install dependencies:**
```bash
ng add @angular/material
npm install @angular/material-luxon-adapter  # For date handling
```
...
public class ContentTypeControllerTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                ...
                {
                    services.AddDbContext<HopenDbContext>(options =>
    ...
    [Fact]
    public async Task CreateContentType_ReturnsCreated()
    {
        var client = _factory.CreateClient();
        var response = await client.PostAsJsonAsync("/api/v1/content-types", 
            new CreateContentTypeDto { Name = "blog_post" });
...
describe('ContentTypesListPage', () => {
  let component: ContentTypesListPage;
  let fixture: ComponentFixture<ContentTypesListPage>;
  let service: jasmine.SpyObj<ContentTypeService>;
  
  beforeEach(() => {
    const serviceSpy = jasmine.createSpyObj('ContentTypeService', ['getAll']);
    
    TestBed.configureTestingModule({
      imports: [ContentTypesListPage],
      providers: [
        { provide: ContentTypeService, useValue: serviceSpy }
      ]
    });
    
    service = TestBed.inject(ContentTypeService) as jasmine.SpyObj<ContentTypeService>;
    fixture = TestBed.createComponent(ContentTypesListPage);
    component = fixture.componentInstance;
  });
  
  it('should load content types on init', async () => {
    const mockData = [{ id: '1', name: 'blog_post' }];
    service.getAll.and.returnValue(Promise.resolve(mockData));
    
    await component.ngOnInit();
    
    expect(component.contentTypes()).toEqual(mockData);
  });
});
```
